    ...
    <div className="min-h-dvh flex flex-col bg-highlight-300 pb-28">
      <header className="sticky top-0 z-10 h-14 bg-light-400 border-b flex items-center justify-center">
        <div className="absolute left-4">
          <Link href="#" className="text-sm text-dark-400">Cancel</Link>
        </div>
        <h1 className="text-[14px] font-bold">Create Bill</h1>
      </header>

      <main className="flex-1 p-4 space-y-4">
        {/* Bill Name */}
        <div className="space-y-1">
          <label className="block text-sm text-dark-400">Bill Name</label>
          <input
            type="text"
        ...
        <section>
          ...
          <div className="rounded-2xl border border-light-600 bg-light-300 divide-y divide-light-600">
            {participants.map((name) => (
              <label key={name} className="flex items-center justify-between px-4 py-3">
